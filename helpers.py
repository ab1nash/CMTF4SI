    ...
    for layer in range(0,1):
        for r in range(1,imSize[0]-1):
            for c in range(1,imSize[1]-1):
                nbrs = img[r+1,c,layer]+img[r-1,c,layer]
                # +img[r,c+1,layer]+img[r,c-1,layer]
                nbrs /= 2
                if abs(img[r,c,layer] - nbrs) > threshold or img[r,c,layer] < 1e-4:
                    mask[r,c] = 1

    for layer in range(0,1):
        for r in [0,imSize[0]-1]:
            for c in [0,imSize[1]-1]:
                if img[r,c,layer] < 1e-4:
                    mask[r,c] = 1
